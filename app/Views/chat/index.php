...
<head>
    ...
    <style>
        ...
        .upload-box input[name=description] {
            width: 100%;
            margin-bottom: .5rem;
        }
...
<body>
    <div class="top">
        <div>Chat de Relatório — Competência
            <?= htmlspecialchars($currentMonthYear, ENT_QUOTES, 'UTF-8')?>
        </div>
        <div>
            <a href="/dashboard" style="color:#fff; margin-right: 12px;">Dashboard</a>
            <form method="post" action="/logout" style="display:inline;"><input type="hidden" name="csrf_token"
                    value="<?= htmlspecialchars($csrfToken ?? '', ENT_QUOTES, 'UTF-8')?>"><button type="submit"
                    style="background:#374151;">Sair</button></form>
        </div>
    </div>
...
endforeach; ?>
            ...
            <form id="chat-form">
                <input id="message" type="text" name="message" placeholder="Descreva uma atividade realizada..."
                    required />
            ...
            <div class="upload-box">
                ...
                <form id="upload-form" enctype="multipart/form-data">
                    ...
                    <input id="description" type="text" name="description"
                        placeholder="Descrição curta da evidência (opcional)" />
...
endforeach; ?>
        ...
        <section class="panel">
            ...
            <p><strong>Status:</strong> <span id="status">
                    <?= htmlspecialchars((string)$report['status'], ENT_QUOTES, 'UTF-8')?>
                </span></p>
            <p><strong>Atualizado em:</strong> <span id="updated-at">
                    <?= htmlspecialchars((string)($report['updated_at'] ?? '-'), ENT_QUOTES, 'UTF-8')?>
                </span></p>
            <pre
                id="draft"><?= htmlspecialchars((string)($report['content_draft'] ?? ''), ENT_QUOTES, 'UTF-8')?></pre>
        </section>
    </div>
