...
<body class="h-full antialiased font-sans text-slate-900" x-data="{ sidebarCollapsed: false }">
    <div class="min-h-full flex">
        <!-- Sidebar -->
        <aside 
            :class="sidebarCollapsed ? 'md:w-20' : 'md:w-64'"
            class="hidden md:flex md:flex-col md:fixed md:inset-y-0 bg-white border-r border-slate-200 transition-all duration-300 z-30">
            <div class="flex flex-col flex-grow pt-5 overflow-y-auto">
                <div class="flex items-center justify-between px-4 mb-8">
                    <span x-show="!sidebarCollapsed" class="text-2xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-primary-600 to-primary-800 truncate">
                        Gestor IA
                    </span>
                ...
                <nav class="flex-1 px-2 space-y-1">
                    <a href="/dashboard" class="group flex items-center px-3 py-2 text-sm font-medium rounded-md text-slate-600 hover:bg-slate-50 hover:text-slate-900" :title="sidebarCollapsed ? 'Dashboard' : ''">
                        <svg class="h-5 w-5 text-slate-400 group-hover:text-slate-500" :class="sidebarCollapsed ? '' : 'mr-3'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                        <span x-show="!sidebarCollapsed">Dashboard</span>
                    </a>

                    <a href="/chat" class="group flex items-center px-3 py-2 text-sm font-medium rounded-md text-slate-600 hover:bg-slate-50 hover:text-slate-900" :title="sidebarCollapsed ? 'Mentoria IA' : ''">
                        <svg class="h-5 w-5 text-slate-400 group-hover:text-slate-500" :class="sidebarCollapsed ? '' : 'mr-3'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z"/></svg>
                        <span x-show="!sidebarCollapsed">Mentoria IA</span>
                    </a>

                    <a href="/reports" class="group flex items-center px-3 py-2 text-sm font-medium rounded-md text-slate-600 hover:bg-slate-50 hover:text-slate-900" :title="sidebarCollapsed ? 'Relatórios' : ''">
                    ...
                    <div x-show="!sidebarCollapsed" class="pt-4 pb-2 px-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Gestão</div>
                    <a href="/team" class="group flex items-center px-3 py-2 text-sm font-medium rounded-md text-slate-600 hover:bg-slate-50 hover:text-slate-900" :title="sidebarCollapsed ? 'Meu Time' : ''">
                        <svg class="h-5 w-5 text-slate-400 group-hover:text-slate-500" :class="sidebarCollapsed ? '' : 'mr-3'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                        <span x-show="!sidebarCollapsed">Meu Time</span>
                    </a>
                    <a href="/team/insights" class="group flex items-center px-3 py-2 text-sm font-medium rounded-md text-slate-600 hover:bg-slate-50 hover:text-slate-900" :title="sidebarCollapsed ? 'Insights do Time' : ''">
                    ...
                    <div x-show="!sidebarCollapsed" class="pt-4 pb-2 px-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Admin</div>
                    <a href="/admin/users" class="group flex items-center px-3 py-2 text-sm font-medium rounded-md text-slate-600 hover:bg-slate-50 hover:text-slate-900" :title="sidebarCollapsed ? 'Usuários' : ''">
                        <svg class="h-5 w-5 text-slate-400 group-hover:text-slate-500" :class="sidebarCollapsed ? '' : 'mr-3'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/></svg>
                        <span x-show="!sidebarCollapsed">Usuários</span>
                    </a>
                    <a href="/admin/personas" class="group flex items-center px-3 py-2 text-sm font-medium rounded-md text-slate-600 hover:bg-slate-50 hover:text-slate-900" :title="sidebarCollapsed ? 'Personas' : ''">
                        <svg class="h-5 w-5 text-slate-400 group-hover:text-slate-500" :class="sidebarCollapsed ? '' : 'mr-3'" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>
                    ...
                    <a href="/profile" class="flex-shrink-0 w-full group block">
                        <div class="flex items-center">
                            ...
                            <div class="ml-3" x-show="!sidebarCollapsed">
                                <p class="text-xs font-bold text-slate-700 truncate max-w-[120px]"><?= htmlspecialchars($user['name']) ?></p>
        ...
        <!-- Main Content -->
        <div class="flex flex-col flex-1 transition-all duration-300" :class="sidebarCollapsed ? 'md:pl-20' : 'md:pl-64'">
            <header class="sticky top-0 z-10 flex-shrink-0 flex h-16 bg-white border-b border-slate-200 shadow-sm">
                <div class="flex-1 px-4 flex justify-between">
                    ...
                    <div class="ml-4 flex items-center md:ml-6 space-x-4">
                        ...
                        <div class="relative" id="notification-dropdown">
                            <button onclick="toggleNotifications()" class="p-2 rounded-full text-slate-400 hover:text-slate-500 hover:bg-slate-100 transition-all relative">
            ...
            <main class="flex-1 overflow-hidden">
                <div class="h-full py-6 px-4 sm:px-6 md:px-8 overflow-y-auto">
                    <?= $slot ?? '' ?>
                </div>
            </main>
        </div>
